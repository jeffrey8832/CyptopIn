...
import { 
  ...
  Settings, Save, Gauge, Star, TrendingUp, Droplets, Layers, ChevronRight, BarChart, Smartphone, Link, Sun, Moon, RefreshCw, Calculator
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const ProfitCalculator: React.FC<{ currentPrice: number, targetPrice?: string }> = ({ currentPrice, targetPrice }) => {
    ...
    // Update internal buy price when prop changes (if user hasn't edited it too much, or just for initial load)
    useEffect(() => {
    ...
    if (b > 0 && p > 0) {
        const amount = p / b;
        totalValue = amount * (s > 0 ? s : b); // If no sell price, assume current value
        if (s > 0) {
            profit = totalValue - p;
            roi = (profit / p) * 100;
        }
    ...
    const useTarget = () => {
        if (targetPrice) {
            // Remove any non-numeric chars like '$' or ','
          ...
          <>
            {viewMode === 'dashboard' ? (
              <div className="space-y-12 animate-fade-in">
                 ...
                 <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-xl relative overflow-hidden transition-colors">
                   <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/5 dark:bg-indigo-500/5 rounded-full filter blur-3xl pointer-events-none"></div>
                   <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-8 mb-8 pb-8 border-b border-slate-100 dark:border-slate-800">
                     <div className="flex items-center gap-5"><img src={currentCoin.image} alt={currentCoin.name} className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-slate-100 dark:bg-slate-800 shadow-xl ring-4 ring-slate-100 dark:ring-slate-800"/><div><h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white flex items-center gap-3">{currentCoin.name} <span className="text-lg font-bold text-indigo-500 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 px-2 py-0.5 rounded-lg border border-indigo-200 dark:border-indigo-500/20">{currentCoin.symbol.toUpperCase()}</span></h2><div className="text-slate-500 dark:text-slate-400 font-medium mt-1 flex items-center gap-2"><span className="bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-xs">Rank #{currentCoin.market_cap_rank}</span></div></div></div>
                     <div className="text-left md:text-right w-full md:w-auto bg-slate-50 dark:bg-slate-800/50 md:bg-transparent p-4 md:p-0 rounded-xl"><div className="text-4xl md:text-5xl font-bold text-slate-900 dark:text-white tracking-tight">{formatPrice(currentCoin.current_price)}</div><div className={`flex items-center md:justify-end gap-1 mt-2 text-lg font-bold ${currentCoin.price_change_percentage_24h >= 0 ? 'text-emerald-500 dark:text-emerald-400' : 'text-rose-500 dark:text-rose-400'}`}>{currentCoin.price_change_percentage_24h >= 0 ? <TrendingUp className="w-5 h-5" /> : <TrendingUp className="w-5 h-5 transform rotate-180" />}{Math.abs(currentCoin.price_change_percentage_24h).toFixed(2)}% (24h)</div></div>
                   </div>
                   <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                      <div className="space-y-1 p-4 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-slate-800/50"><div className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.marketCap}</div><div className="text-lg font-bold text-slate-900 dark:text-slate-200">{formatCurrency(currentCoin.market_cap)}</div></div>
                      <div className="space-y-1 p-4 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-slate-800/50"><div className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.volume24h}</div><div className="text-lg font-bold text-slate-900 dark:text-slate-200">{formatCurrency(currentCoin.total_volume)}</div></div>
                      <div className="space-y-1 p-4 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-slate-800/50"><div className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.high24h}</div><div className="text-lg font-bold text-slate-900 dark:text-slate-200">{formatPrice(currentCoin.high_24h)}</div></div>
                      <div className="space-y-1 p-4 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-slate-800/50"><div className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.low24h}</div><div className="text-lg font-bold text-slate-900 dark:text-slate-200">{formatPrice(currentCoin.low_24h)}</div></div>
                   </div>
                   
                   <div className="bg-slate-50 dark:bg-slate-800/20 rounded-2xl p-4 border border-slate-100 dark:border-slate-800/50 h-[360px] flex flex-col">
              ...
              <div className="max-w-4xl mx-auto space-y-6 animate-fade-in-up">
                ...
                <div className="grid md:grid-cols-3 gap-6">
                   ...
                   {/* Key Indicators Bionic Grid (Spans 3 Rows to account for ShortTerm + Calculator) */}
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-md transition-colors">
                   <h3 className="text-lg font-bold text-indigo-500 dark:text-indigo-400 mb-4 flex items-center gap-2"><Newspaper className="w-5 h-5" /> {t.latestNews}</h3>
                   <div className="grid gap-3">{news.length > 0 ? news.map((item, idx) => (<a key={idx} href={item.url} target="_blank" rel="noreferrer" className="block p-4 bg-slate-50 dark:bg-slate-800/30 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800/50 hover:border-slate-300 dark:hover:border-slate-700 rounded-xl transition-all group"><div className="flex justify-between items-start gap-4"><div><h4 className="text-slate-800 dark:text-slate-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-300 font-semibold text-sm mb-1 leading-snug">{item.title}</h4><div className="flex items-center gap-2 mt-1"><span className="text-xs text-slate-500 bg-slate-200 dark:bg-slate-800 px-1.5 py-0.5 rounded">{item.author}</span><span className="text-xs text-slate-500 dark:text-slate-600">•</span><span className="text-xs text-slate-500 dark:text-slate-600">{new Date(item.created_at).toLocaleDateString()}</span></div></div><ArrowUpRight className="w-4 h-4 text-slate-500 dark:text-slate-600 group-hover:text-indigo-500 dark:group-hover:text-indigo-400 opacity-0 group-hover:opacity-100 transition-all flex-shrink-0" /></div></a>)) : <div className="text-slate-500 text-sm py-8 text-center bg-slate-50 dark:bg-slate-800/20 rounded-xl">Loading news...</div>}</div>
                </div>
              </div>
            )}
          </>
